...
""", dry=getattr(args, 'dry_run', False))
        ...
        wrote = init_persona_manifests(overwrite='--overwrite' in sys.argv, dry=False)
        print(f"--------personas:init: wrote {wrote} manifests under personas/")
        return 0
    elif args.cmd == "personas:build":
        print("\033[36m📋 Building personas for Cursor (.cursor) and Codex (.codex)...\033[0m")
        install_cursor_commands_in_project(False)
        write_codex_agent_assets(False)
        print(f"\033[32m✓\033[0m \033[1mDone:\033[0m .cursor commands + .codex/agents catalog updated")
        return 0
    elif args.cmd == 'sdd':
        print_mcp_banner()
