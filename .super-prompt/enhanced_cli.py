...
VERSION = "3.1.70-enhanced"  # Enhanced unified version
...
🚨 This tag must be executed directly by Cursor AI, not external CLI."""
    ...
    def detect_tag(self, input_text: str) -> Optional[str]:
        """
        Detect persona tags in priority order:
        1. --sp-[persona] (Super Prompt standard format)
        2. --persona-[persona] (legacy format)
        3. /[persona] (shorthand format)
        4. --[persona] (direct format)
        """
        # Check for debate-interactive first (more specific)
        if '--sp-debate-interactive' in input_text or '/debate-interactive' in input_text or '--debate-interactive' in input_text:
            return 'debate-interactive'

        # Check for seq-ultra first (more specific than seq)
        if '--sp-seq-ultra' in input_text or '/seq-ultra' in input_text or '--seq-ultra' in input_text:
            return 'seq-ultra'

        # Primary: --sp-[persona] format (Super Prompt standard)
        sp_match = re.search(r'--sp-(\w+)', input_text)
        if sp_match:
            persona = sp_match.group(1)
            if persona in self.PERSONAS or persona in ['seq', 'seq-ultra', 'high', 'debate', 'debate-interactive']:
                return persona

        # Secondary: Check all persona patterns
        for persona in self.PERSONAS:
            if (f'--sp-{persona}' in input_text or
                f'--persona-{persona}' in input_text or
                f'/{persona}' in input_text):
                return persona

        # Legacy support for specific tags
        if re.search(r'--sp-seq($|\s)', input_text) or re.search(r'--seq($|\s)', input_text):
            return 'seq'
        elif '--sp-high' in input_text or '--high' in input_text:
            return 'high'
        elif '--sp-debate' in input_text or '--debate' in input_text:
            return 'debate'

        return None

    def clean_input(self, input_text: str) -> str:
        """Clean persona tags from input text, supporting all formats"""
        cleaned = input_text

        # Handle debate-interactive first (more specific)
        cleaned = re.sub(r'--sp-debate-interactive|/debate-interactive|--debate-interactive', '', cleaned)

        # Handle seq-ultra before seq (more specific)
        cleaned = re.sub(r'--sp-seq-ultra|/seq-ultra|--seq-ultra', '', cleaned)

        # Clean --sp-[persona] patterns (primary format)
        cleaned = re.sub(r'--sp-\w+', '', cleaned)

        # Clean legacy patterns
        for persona in self.PERSONAS:
            cleaned = re.sub(f'/{persona}|--persona-{persona}', '', cleaned)

        # Clean remaining specific patterns
        cleaned = re.sub(r'--seq(?=\s|$)', '', cleaned)
        cleaned = re.sub(r'--high(?=\s|$)', '', cleaned)
        cleaned = re.sub(r'--debate(?=\s|$)', '', cleaned)
