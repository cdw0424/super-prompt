...
class EnhancedPersonaProcessor:
    ...
    def generate_system_prompt(self, persona_key: str, user_input: str, external_plan: Optional[dict] = None) -> str:
        ...
        import os as _os
        codex_mode = _os.environ.get('SP_CODEX_MODE') == '1'
        codex_block = ("\n## CODEX AMR MODE — GPT-5 WORKFLOW PRINCIPLES\n" + self._get_codex_mode_guidelines() + "\n") if codex_mode else ""
...
{self._get_global_prompt_engineering_rules()}{grok_block}{codex_block}
...
and follow your defined interaction style throughout the entire response."""
    ...
    def _get_codex_mode_guidelines(self) -> str:
        """Codex mode specific prompting/principles (aligned with persona_make_rules)."""
        return (
            "- Reasoning effort: start medium; escalate to high for multi-step or uncertain tasks; de-escalate for execution.\n"
            "- Verbosity: keep narrative low; allow high detail inside code/diff/test blocks.\n"
            "- Tool preambles: before each tool call, restate goal and outline the plan briefly.\n"
            "- Status updates: while executing, emit concise progress notes tied to each call; after, separate 'planned' vs 'did'.\n"
            "- Planning: front-load an explicit plan and clear tool instructions; include a short rationale (bullets) in final answer; no chain-of-thought.\n"
            "- Context gathering: one parallel batch → read top hits only → dedupe; trace only symbols to change/call; escalate once if signals conflict.\n"
            "- Instruction hygiene: remove contradictions/ambiguities; define stop conditions and when user confirmation is required.\n"
            "- Coding performance: prefer small, focused diffs; low text verbosity + high code verbosity; match existing project norms.\n"
            "- Steering: use /model gpt-5 high for planning/review then /model gpt-5 medium for execution (or equivalent router logs).\n"
            "- Markdown: use only when helpful (inline code, fences, lists, tables); keep logs prefixed with '--------'.\n"
            "- Operational safety: apply patch, verify thoroughly, and avoid destructive ops without explicit consent when the domain requires it.\n"
        )
...
Be thorough, insightful, and provide actionable intelligence."""
    ...
    def execute_persona(self, persona_key: str, user_input: str) -> None:
        ...
        # Check if we should force high reasoning in Grok/Codex modes
        import os
        grok_mode = os.environ.get('SP_GROK_MODE') == '1'
        codex_mode = os.environ.get('SP_CODEX_MODE') == '1'
        mode_force_high = (grok_mode or codex_mode) and self._should_force_high_reasoning(persona_key, user_input)
        ...
        needs_deep_reasoning = (
            force_gpt5_high
            or mode_force_high
            or getattr(self, "_force_delegate", False)
            or self._needs_deep_reasoning(persona_key, user_input)
        )

        if needs_deep_reasoning:
            try:
                if mode_force_high:
                    which = "Grok" if grok_mode else "Codex AMR"
                    print(f"-------- 🧠 {which} mode: Force high reasoning for selected commands")
                else:
...
"""
    ...
    def _needs_deep_reasoning(self, persona_key: str, user_input: str) -> bool:
        ...
        # Additional keywords when in Grok/Codex modes (more aggressive reasoning)
        grok_keywords = [
        ...
        # In Grok or Codex mode, also check for additional reasoning keywords
        codex_mode = os.environ.get('SP_CODEX_MODE') == '1'
        if (grok_mode or codex_mode) and any(k in s for k in grok_keywords):
            which = "Grok" if grok_mode else "Codex AMR"
            print(f"-------- 🧠 {which} mode: Enhanced reasoning triggered by keyword detection")
            return True
        ...
        # In Grok/Codex modes, expand to more personas
        if grok_mode or codex_mode:
            high_reasoning_personas.update({"backend", "frontend", "dev", "mentor", "refactorer"})
            which = "Grok" if grok_mode else "Codex AMR"
            print(f"-------- 🧠 {which} mode: Expanded reasoning scope for persona '{persona_key}'")

        return persona_key in high_reasoning_personas

    def _should_force_high_reasoning(self, persona_key: str, user_input: str) -> bool:
        """Determine if we should force high reasoning for ALL commands in Grok/Codex modes."""
        # In Grok/Codex mode, force high reasoning for commands that involve:
        ...
        # Also force for certain personas in Grok/Codex modes
