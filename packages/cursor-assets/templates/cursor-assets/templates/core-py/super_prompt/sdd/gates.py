...
SPEC_REQUIRED_SECTIONS = [
    'REQ-ID', 'Overview', 'Success Criteria', 'Acceptance Criteria',
    'Scope & Boundaries', 'Business Value'
]

PLAN_REQUIRED_SECTIONS = [
    'REQ-ID', 'Architecture Overview', 'Technical Stack', 'Security Architecture',
    'Testing Strategy', 'Deployment Strategy', 'Success Metrics'
]

TASKS_REQUIRED_SECTIONS = [
    'REQ-ID', 'Task Breakdown Strategy', 'Acceptance Self-Check Template'
]
...
def check_spec_plan() -> GateReport:
    ...
    spec_files = list(_find_files('specs', 'spec.md'))
    plan_files = list(_find_files('specs', 'plan.md'))
    ...
    latest_spec = max(spec_files, key=lambda p: Path(p).stat().st_mtime)
    latest_plan = max(plan_files, key=lambda p: Path(p).stat().st_mtime)
    ...
    constitution_check = _check_constitution_compliance(latest_spec, latest_plan)
...
def check_tasks() -> GateReport:
    """Check tasks implementation readiness"""
    miss = []
    warnings = []

    tasks_files = list(_find_files('specs', 'tasks.md'))
    if not tasks_files:
        miss.append('tasks.md file (run /tasks command first)')
        return GateReport(ok=False, missing=miss)

    latest_tasks = max(tasks_files, key=lambda p: Path(p).stat().st_mtime)
...
def check_implementation_ready() -> GateReport:
    """Final gate before implementation - validates all SDD requirements"""
    miss = []
    warnings = []

    # Check all previous gates
    spec_plan_check = check_spec_plan()
    tasks_check = check_tasks()
    ...
    acceptance_check = _validate_acceptance_self_check()
    ...
    if acceptance_check.ok:
        self_check_result = _run_acceptance_self_check()
...
def _validate_spec_quality(spec_path: str) -> GateReport:
...
def _validate_plan_quality(plan_path: str) -> GateReport:
...
def _validate_tasks_quality(tasks_path: str) -> GateReport:
    ...
    try:
        ...
        if not re.search(r'TASK-[\w-]+.*Description.*Acceptance Criteria', content, re.DOTALL):
            miss.append('Tasks must follow proper TASK-ID format with descriptions and acceptance criteria')
...
def _check_constitution_compliance(spec_path: str, plan_path: str) -> ConstitutionCheck:
    """Check compliance with project constitution"""
    violations = []
    recommendations = []

    constitution_path = 'memory/constitution/constitution.md'
    ...
    try:
        ...
        if 'SPEC → PLAN → TASKS → IMPLEMENT' in constitution:
            if not ('Success Criteria' in spec_content and 'Acceptance Criteria' in spec_content):
                violations.append('Spec must include success and acceptance criteria per constitution')
...
def _validate_acceptance_self_check() -> GateReport:
    ...
    try:
        ...
        required_functions = ['validate_success_criteria', 'validate_acceptance_criteria', 'generate_check_report']
...
def _run_acceptance_self_check() -> GateReport:
    ...
    try:
        ...
        result = subprocess.run([
            'python3', 'scripts/sdd/acceptance_self_check.py', '--quiet'
...
def _calculate_quality_score(spec_quality: GateReport, plan_quality: GateReport,
                           constitution_check: ConstitutionCheck) -> int:
