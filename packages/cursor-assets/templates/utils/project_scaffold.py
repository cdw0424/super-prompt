...
def install_cursor_commands_in_project(dry: bool = False):
    ...
    # FORCE ADVANCED VERSION: Always try to find and use the advanced tag-executor.sh
    import shutil
    import pathlib

    # Try multiple possible locations for the advanced version
    possible_paths = [
        # From packages/cursor-assets/templates (in distributed package)
        pathlib.Path(__file__).parent.parent.parent / "packages" / "cursor-assets" / "templates" / "tag-executor.sh",
        # From current working directory
        pathlib.Path.cwd() / "packages" / "cursor-assets" / "templates" / "tag-executor.sh",
        # From script directory
        pathlib.Path(__file__).parent.parent.parent.parent / "packages" / "cursor-assets" / "templates" / "tag-executor.sh",
    ]

    advanced_found = False
    for advanced_tag_sh in possible_paths:
        if advanced_tag_sh.exists():
            if not dry:
                shutil.copy2(str(advanced_tag_sh), os.path.join(base, 'tag-executor.sh'))
                # Make it executable
                os.chmod(os.path.join(base, 'tag-executor.sh'), 0o755)
            advanced_found = True
            break

    # If no advanced version found, create a minimal but functional version
    if not advanced_found:
        tag_sh = """#!/usr/bin/env bash
set -euo pipefail

# Advanced tag-executor.sh not found - using fallback
# This fallback version provides basic functionality
