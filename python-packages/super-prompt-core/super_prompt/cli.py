...
from .mcp_register import ensure_cursor_mcp_registered, validate_project_ssot, ensure_project_ssot_priority, get_ssot_guidance, perform_ssot_migration
...
def setup_global(
    ...
    """Explicit global MCP setup (SSOT compliance)."""
    if not cursor:
        typer.echo("❌ Error: Must specify --cursor to run global setup", err=True)
        raise typer.Exit(1)
...
def setup_cleanup(
    ...
    if not cursor:
        typer.echo("❌ Error: Must specify --cursor to run cleanup", err=True)
        raise typer.Exit(1)
...
def setup_migrate(
    ...
    try:
        ...
        typer.echo(f"     • Codex CLI: {'ℹ️ User-managed' if True else '❌'}")
...
@app.command("sdd-mode-on")
def sdd_mode_on(
    project_root: str = typer.Option(".", "--project-root", help="Project root directory"),
):
    """Enable SDD mode by creating .cursor/.sdd-mode flag."""
    try:
        root = Path(project_root)
        cursor_dir = root / ".cursor"
        cursor_dir.mkdir(parents=True, exist_ok=True)
        flag = cursor_dir / ".sdd-mode"
        flag.write_text("", encoding="utf-8")
        typer.echo("-------- SDD mode: ENABLED (.cursor/.sdd-mode)")
    except Exception as e:
        typer.echo(f"❌ Error enabling SDD mode: {e}", err=True)
        raise typer.Exit(1)


@app.command("sdd-mode-off")
def sdd_mode_off(
    project_root: str = typer.Option(".", "--project-root", help="Project root directory"),
):
    """Disable SDD mode by removing .cursor/.sdd-mode flag."""
    try:
        root = Path(project_root)
        flag = root / ".cursor" / ".sdd-mode"
        if flag.exists():
            flag.unlink()
            typer.echo("-------- SDD mode: DISABLED")
        else:
            typer.echo("-------- SDD mode: Already disabled")
    except Exception as e:
        typer.echo(f"❌ Error disabling SDD mode: {e}", err=True)
        raise typer.Exit(1)
...
def init(
    ...
    - .codex/ (Codex CLI configuration - managed by user)
    These directories are protected and should only be modified by their respective official tools.
    """
    # 디버깅 메시지 (함수 시작 부분)
    import sys
    import os
    ...
    try:
        ...
        typer.echo("\033[33m  - .codex/ (Codex CLI configuration - managed by user)\033[0m")
        typer.echo(
...
"""
        ...
        # Codex functionality has been removed to prevent .codex file modifications
...
"""
        ...
        # NOTE: Codex functionality has been removed to prevent .codex file modifications during super:init
