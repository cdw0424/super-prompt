...
"""
        ...
        # Detect and preserve current mode, or set default to GPT
        try:
            cursor_dir = target_dir / ".cursor"

            # Check for existing mode flags
            current_mode = None
            if (cursor_dir / ".grok-mode").exists():
                current_mode = "grok"
            elif (cursor_dir / ".codex-mode").exists():
                current_mode = "gpt"

            # Preserve existing mode or set default
            if current_mode:
                set_mode_file(current_mode)
                typer.echo(f"✅ Preserved existing mode: {current_mode} (.super-prompt/mode.json)")
            else:
                set_mode_file("gpt")
                typer.echo("✅ Default LLM mode set to gpt (.super-prompt/mode.json)")
        except Exception as e:
            typer.echo(f"⚠️  Could not set mode: {e}")
