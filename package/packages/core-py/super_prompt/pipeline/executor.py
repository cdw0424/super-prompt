...
def _build_plan_lines_from_state(persona: str, state: PipelineState) -> List[str]:
    ...
    # Codex insights removed - using Cursor-only approach
...
def _build_exec_lines_from_state(persona: str, state: PipelineState) -> List[str]:
    ...
    # Codex validation removed - using Cursor-only approach
    relevance = state.context_info.get("query_relevance") or []
    if relevance:
...
def run_persona_pipeline(
    ...
    try:
        ...
        # Codex assistance removed - using Cursor-only approach
        ...
        # Codex functionality disabled
        codex_needed = False
        codex_response: Optional[str] = None
        # Removed: ctx_hint = f"Patterns: {ctx_patterns}" if ctx_patterns else ""
        # Removed: codex_response = call_codex_assistance(query, ctx_hint, config.persona)
        ...
        state = PipelineState(
            ...
            codex_response=None,  # Codex disabled
            ...
            try:
                ...
                store.append_event(
                    config.memory_tag,
                    {
                        ...
                        "codex_used": False,  # Codex disabled
                        "state": {
                            "relevance": context_info.get("query_relevance", []),
                            "codex": False,  # Codex disabled
        ...
        try:
            ...
            kv_items = {
                ...
                f"pipeline:has_codex:{config.persona}": "0",  # Codex disabled
        ...
        # Codex insight removed - using Cursor-only approach
