...
# Function to process grok commands cleanly
process_grok_command() {
    local grok_command="$1"

    case "$grok_command" in
        "grok-on")
            # SECURITY EXCEPTION: grok-on is explicitly allowed to create .cursor directory
...
for arg in "$@"; do
    ...
    # PRIORITY 2: Check for explicit --sp- format
        ...
        # Special handling for grok toggle commands - process immediately and exit
        if [[ "$DETECTED_TAG" == "grok-on" ]] || [[ "$DETECTED_TAG" == "grok-off" ]]; then
            TAG="$DETECTED_TAG"
            COMMAND_DETECTED=true
            COMMAND_EXECUTION_GUARANTEED=true
            echo "-------- ⚡ EXECUTING GROK COMMAND IMMEDIATELY" >&2
            process_grok_command "$TAG"
            exit 0
        else
            TAG="$DETECTED_TAG"
            COMMAND_DETECTED=true
...
# Skip grok mode processing for grok-on/off commands (already handled above)
if [[ "$TAG" != "grok-on" ]] && [[ "$TAG" != "grok-off" ]]; then
    # Ensure GROK_FLAG_FILE is properly set
...
# Display Grok mode status in execution
if [ -f "$GROK_FLAG_FILE" ]; then
